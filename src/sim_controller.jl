...
"""
function create_prototypes(context_labels::Vector{Symbol}, hypothesis_labels::Vector{Symbol},
                           n_features::Integer=15, focal_sim::Number=0.0)
    ...
    focal_prototype = MemoryTrace(hypothesis_labels[1], string(hypothesis_labels[1])*" hypothesis", focal_contexts)
    
    # Replicate the focal hypothesis
    alt_prototypes = Vector{MemoryTrace}()
    for alt in 2:length(hypothesis_labels)
...
"""
function create_prototypes(n_contexts::Integer, n_hypotheses::Integer, n_features::Integer=15, focal_sim::Number=0.0)
    # Create labels
...
function create_labels(n_labels::Integer, label::String)
    return [Symbol(label*string(i)) for i in 1:n_labels]
end
...
"""
function create_traces(prototypes::Vector{MemoryTrace}, n_trace_vec::Vector{<:Number},
                       encode_fidelity::Number=0.75)
    traces = Vector{MemoryTrace}()
    for (proto, n) in zip(prototypes, n_trace_vec)
        [push!(traces, trace_replication(proto, encode_fidelity)) for _ in 1:n]
    end
    return traces
end
...
"""
function create_traces(prototypes::Vector{MemoryTrace}, n_traces::Integer,
                       encode_fidelity::Number=0.75)
