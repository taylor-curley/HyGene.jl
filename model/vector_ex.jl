...
function sim_controller(n_features, n_minivecs, n_hypotheses, t_max, A_c, S, L, d_obs_sim, trace_dists, n_trials)
    ...
    alt_prototypes = [trace_replication(focal_prototype, S) for _ in 1:(n_hypotheses-1)]
...
for _ in 1:n_participants
    for S in focal_sim
        for L in encode_fidelity
            out = sim_controller(n_features, n_minivecs, n_hypotheses, t_max, A_c, S, L, d_obs_sim, trace_dists, n_trials)
...
a = @df sim_avgs[sim_avgs.L.==0.1,:] plot(:S, :ACC_mean,
        lims = (0,1), grid = false, title = "L = 0.1")
b = @df sim_avgs[sim_avgs.L.==0.3,:] plot(:S, :ACC_mean, 
        lims = (0,1), grid = false, title = "L = 0.3")
c = @df sim_avgs[sim_avgs.L.==0.5,:] plot(:S, :ACC_mean,
        lims = (0,1), grid = false, title = "L = 0.5")
d = @df sim_avgs[sim_avgs.L.==0.7,:] plot(:S, :ACC_mean,
        lims = (0,1), grid = false, title = "L = 0.7")
e = @df sim_avgs[sim_avgs.L.==0.9,:] plot(:S, :ACC_mean,
        lims = (0,1), grid = false, title = "L = 0.9")
f = @df sim_avgs[sim_avgs.L.==1.0,:] plot(:S, :ACC_mean, 
        lims = (0,1), grid = false, title = "L = 1.0")
plot(a,b,c,d,e,f, layout = lay, size = (1_200, 300), plot_title = "Average Accuracy", xlabel = "S")

g = @df sim_avgs[sim_avgs.L.==0.1,:] plot(:S, :N_SOC_mean, 
        ylims = (0,3), grid = false, title = "L = 0.1")
h = @df sim_avgs[sim_avgs.L.==0.3,:] plot(:S, :N_SOC_mean, 
        ylims = (0,3), grid = false, title = "L = 0.3")
i = @df sim_avgs[sim_avgs.L.==0.5,:] plot(:S, :N_SOC_mean, 
        ylims = (0,3), grid = false, title = "L = 0.5")
j = @df sim_avgs[sim_avgs.L.==0.7,:] plot(:S, :N_SOC_mean, 
        ylims = (0,3), grid = false, title = "L = 0.7")
k = @df sim_avgs[sim_avgs.L.==0.9,:] plot(:S, :N_SOC_mean, 
        ylims = (0,3), grid = false, title = "L = 0.9")
l = @df sim_avgs[sim_avgs.L.==1.0,:] plot(:S, :N_SOC_mean, 
        ylims = (0,3), grid = false, title = "L = 1.0")
plot(g,h,i,j,k,l, layout = lay, size = (1_200, 300), plot_title = "Number of Contenders", xlabel = "S")
